...
?>
<?php Pjax::begin();?>    
    <?= Html::beginForm(Url::to(''), 'post', ['data-pjax' => ''])?>
    ...
    <?php if($dataProvider):?>
            ...
            <?php foreach ($dataProvider->getModels() as $k => $v):?>
                <div style="cursor: pointer" onclick='javascript: document.location.href = "<?=$v['link']?>"' class="news_list_day_item clear">
                    <div class="news_list_search_result clear">
                        <div class="news_list_day_item_picture">
                            <img src='<?=$v['photo']?>' alt="">
                        </div>
                        <div class="news_list_day_item_content">
                            <div class="news_list_day_item_content_title"><?= mb_substr($v['name'], 0,30,'utf8')?></div>
